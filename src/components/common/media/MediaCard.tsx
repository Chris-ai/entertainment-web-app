...
const Category: React.FC<{ category: "Movie" | "TV Series" }> = ({
  category,
}) => {
  switch (category) {
    case "Movie":
...
const MediaInfo: React.FC<{
  ...
  return (
    <div
      className={
        "flex gap-x-1.5 text-white text-opacity-75 justify-start items-center text-xs md:text-bm"
      }
    >
      <p>{year}</p>
...
const MediaCard: React.FC<IProps> = ({ mediaElement, variant = "default" }) => {
  const [mediaElementBookmark, setMediaElementBookmark] = useState<boolean>(
    mediaElement.isBookmarked
  );
  ...
  if (variant === "large") {
    return (
      <div
        className={
          "flex flex-col justify-between w-[240px] md:w-[470px] relative"
        }
      >
        <MediaLargeCardImage thumbnail={mediaElement.thumbnail} />
        <BookmarkButton
          isBookmarked={mediaElementBookmark}
          onClick={handleBookmarkMedia}
        />
        <div
          className={
            "absolute left-4 bottom-4 md:left-6 md:bottom-6 flex flex-col gap-y-1.5"
          }
        >
          <div
            className={
              "flex md:hidden gap-x-1.5 text-white text-opacity-75 justify-start items-center text-xs"
            }
          >
          ...
          <div className={"hidden md:block"}>
            <MediaInfo
              ...
              rating={mediaElement.rating}
            />
          </div>
          <p
            className={
              "text-[0.875rem] md:font-medium md:text-hs md:leading-normal"
            }
          >
            {mediaElement.title}
          </p>
        ...
        <div
          className={
            "absolute right-4 bottom-4 md:right-6 md:bottom-6 block md:hidden"
          }
        >
